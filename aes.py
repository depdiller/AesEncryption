...
assert (
    substitution(bytearray.fromhex('12')).hex() == 'c9'
)
assert (
    substitution(bytearray.fromhex('fd')).hex() == '54'
)
        
def inv_substitution(data):
    new_block = bytearray()
    for byte in data:
        i = byte // 16
        j = byte % 16
        new_block.append(inv_s_box[i][j])
    return new_block

assert (
    inv_substitution(bytearray.fromhex('c9')).hex() == '12'
)
assert (
    inv_substitution(bytearray.fromhex('54')).hex() == 'fd'
)
...
def decrypt_block(data, key):
    length_diff = BLOCK_SIZE - len(data)
    if (length_diff != 0):
        raise Exception('Data block for decryption must be = 32 bits')
    data = xor(data, key)
    data = permutation(data)
    decrypted_block = inv_substitution(data)
    return decrypted_block

data = bytearray.fromhex('12 34 d1 a1')
key = bytearray.fromhex('ad d2 13 da')
enc = encrypt_block(data, key)
